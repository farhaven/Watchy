...
GxEPD2_BW<WatchyDisplay, WatchyDisplay::HEIGHT> Watchy::display(WatchyDisplay(DISPLAY_CS, DISPLAY_DC, DISPLAY_RES, DISPLAY_BUSY));
...
void Watchy::showTextEntry() {
	...
	char enteredText[16] = {};
	memset(enteredText, ' ', sizeof(enteredText));
	enteredText[sizeof(enteredText) - 1] = 0;
	...
	while (1) {
		...
		display.println("Entered text:");
		display.println(enteredText);

		display.drawChar(position * (7 + 4), 3 * (9 + 1), '_', GxEPD_BLACK, GxEPD_WHITE, 1);
		...
		if (digitalRead(BACK_BTN_PIN) == 1) {
			...
			if (position < sizeof(enteredText) - 2) { // Og en til C++ nissen!
				position++;
			} else {
				position = 0;
			}
		} else if (digitalRead(UP_BTN_PIN) == 1) {
			enteredText[position]--;
			if (enteredText[position] < ' ') {
				enteredText[position] = '~';
			}
		} else if (digitalRead(DOWN_BTN_PIN) == 1) {
			enteredText[position]++;
